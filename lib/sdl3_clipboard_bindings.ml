...
let set_text = ff "SDL_SetClipboardText"
  (string @-> returning true_to_ok)

let get_text = ff "SDL_GetClipboardText"
  (void @-> returning string)

let has_text = ff "SDL_HasClipboardText"
  (void @-> returning bool)

let set_data = ff "SDL_SetClipboardData"
  (clipboard_data_callback @-> clipboard_cleanup_callback @-> ptr void @-> ptr string @-> size_t @-> returning true_to_ok)
let set_data callback cleanup userdata mime_types =
  let mime_types, num_mime_types = carray_of_list string mime_types in
  set_data callback cleanup userdata mime_types (Unsigned.Size_t.of_int num_mime_types)

let clear_data = ff "SDL_ClearClipboardData"
  (void @-> returning true_to_ok)

let get_data = ff "SDL_GetClipboardData"
  (string @-> ptr size_t @-> returning (ptr void))

let has_data = ff "SDL_HasClipboardData"
  (string @-> returning bool)

let get_mime_types = ff "SDL_GetClipboardMimeTypes"
...
let get_mime_types () =
  ...
  let p = get_mime_types num_mime_types in
  if is_null p then []
