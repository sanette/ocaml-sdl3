...
Some SDL functions take a function as a argument, to be called later
...
really be disposable. Therefore you shoud "save" the function, that is
to say, register it in a variable that you know will be kept alive by
the OCaml GC long enough.
...
initialized. In particular, the "position" field is created. Threfore
...
1. When creating a Vertex, you should not use the '~position'
   ...
   probably never be freed. (Well, it's not very important because the
   additional memory usage is here quite small). Instead, modify the
...
2. Most importantly, you should never update the position with a new
   one using `Vertex.(set position new_position)` because the old
   position will never be freed, and if you do this in a loop you will
   have a memory leak. Instead, mutate the position 'in-place' as
   before.
